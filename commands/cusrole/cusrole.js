...
module.exports = {
  ...
  async exec(client, message, args) {
    ...
    if (sub === 'up') {
      ...
      let roleType;
      if (isBoost && isDonate) {
        await message.channel.send("<a:Crown:1414236350135144648> **|** Kamu punya role **Boost** dan **Donate**. Ketik `boost` atau `donate` untuk memilih custom role yang ingin dinaikkan.");
        const collected = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] });
        const typeInput = collected.first().content.toLowerCase();
        if (!['boost', 'donate'].includes(typeInput)) {
          return message.channel.send("<a:Crown:1414236350135144648> **|** Tipe tidak valid. Harus `boost` atau `donate`.");
        }
        roleType = typeInput;
      } else if (isBoost) {
        roleType = 'boost';
        ...
        return message.reply("<a:Crown:1414236350135144648> **|** Anda tidak memiliki role Boost atau Donate.");
      }
      ...
      if (!customRole) {
        return message.reply(`<a:Crown:1414236350135144648> **|** Anda belum memiliki custom role bertipe **${roleType}**.`);
      }
      const role = message.guild.roles.cache.get(customRole.roleId);
      if (!role) {
        return message.reply('<a:Crown:1414236350135144648> **|** Role custom tidak ditemukan di server.');
      }
      if (!role.editable) {
        return message.reply('<a:Crown:1414236350135144648> **|** Saya tidak memiliki izin untuk mengatur posisi role ini.');
      }
      ...
      if (!highest) {
        return message.reply('<a:Crown:1414236350135144648> **|** Tidak ada role lain yang bisa dijadikan referensi.');
      }
      try {
        await role.setPosition(highest.position + 1);
        return message.channel.send(`<a:Crown:1414236350135144648> **|** Role custom kamu telah dinaikkan di atas <@&${highest.id}>.`);
      } catch (err) {
        console.error(err);
        return message.channel.send('<a:Crown:1414236350135144648> **|** Gagal menaikkan role. Pastikan bot memiliki izin dan role bot lebih tinggi.');
      }
    } else if (sub === 'create') {
      const filter = m => m.author.id === message.author.id;
      ...
      let roleType;
      if (isBoost && isDonate) {
        await message.channel.send("<a:Crown:1414236350135144648> **|** Kamu memiliki role **Boost** dan **Donate**. Ketik `boost` atau `donate` untuk memilih jenis custom role yang ingin dibuat.");
        const collected = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] });
        const typeInput = collected.first().content.toLowerCase();
        if (!['boost', 'donate'].includes(typeInput)) {
          return message.channel.send("<a:Crown:1414236350135144648> **|** Tipe tidak valid. Harus `boost` atau `donate`.");
      ...
      if (existingRole) {
        return message.reply(`<a:Crown:1414236350135144648> **|** Anda sudah memiliki custom role bertipe **${roleType}**.`);
      }

      try {
        // Step 1: Ask for role name
        await message.channel.send('<a:Crown:1414236350135144648> **|** Silakan ketik nama role yang kamu inginkan (dalam 30 detik)...');
        ...
        await message.channel.send('<a:Crown:1414236350135144648> **|** Ketik warna role dalam format HEX (contoh: `#FF0000`) atau dua kode HEX untuk gradient (contoh: `#34ebd8 #34eb77`)');
        ...
        if (colorParts.length === 1) {
          if (!/^#([0-9A-F]{6})$/i.test(roleColor)) {
            return message.channel.send('<a:Crown:1414236350135144648> **|** Format warna tidak valid. Gunakan format HEX seperti `#RRGGBB`.');
          }
        } else if (colorParts.length === 2) {
          if (!/^#([0-9A-F]{6})$/i.test(colorParts[0]) || !/^#([0-9A-F]{6})$/i.test(colorParts[1])) {
            return message.channel.send('<a:Crown:1414236350135144648> **|** Kedua warna harus format HEX seperti `#RRGGBB #RRGGBB`.');
          }
          gradientColor = colorParts[1];
        } else {
          return message.channel.send('<a:Crown:1414236350135144648> **|** Masukkan satu atau dua kode warna HEX.');
        }

        // Step 3: Ask for icon
        await message.channel.send('<a:Crown:1414236350135144648> **|** Kirim gambar untuk icon role kamu (hanya image), atau ketik `cancel` untuk lewati (30 detik)...');
        ...
        async function createRoleWithColor(options) {
          if (gradientColor && typeof options.setColors === 'function') {
            ...
            try {
              ...
              await message.channel.send('<a:Crown:1414236350135144648> **|** Gagal mengatur gradient, menggunakan warna utama saja.');
            }
            return r;
          } else {
        ...
        if (iconContent === 'cancel' || !hasAttachment) {
          ...
          await message.channel.send('<a:Crown:1414236350135144648> **|** Role berhasil dibuat tanpa icon.');
        } else {
          const attachment = iconMsg.first().attachments.first();
          if (!attachment.contentType?.startsWith('image')) {
            return message.channel.send('<a:Crown:1414236350135144648> **|** File yang dikirim bukan gambar.');
          }
          ...
          try {
            ...
            console.warn('<a:Crown:1414236350135144648> **|** Gagal menetapkan icon role:', err.message);
            message.channel.send('<a:Crown:1414236350135144648> **|** Role berhasil dibuat, tapi gagal mengatur icon (mungkin karena batasan level boost server).');
          }
        }
        ...
        try {
          ...
          message.channel.send('<a:Crown:1414236350135144648> **|** Role berhasil dibuat, namun gagal memberikan role ke Anda.');
        }

        await message.channel.send(`<a:Crown:1414236350135144648> **|** Role berhasil dibuat: <@&${role.id}>`);

      } catch (err) {
        console.error(err);
        message.channel.send('<a:Crown:1414236350135144648> **|** Waktu habis atau terjadi kesalahan. Silakan coba lagi.');
      }
    }
        ...
        let roleType;
        if (isBoost && isDonate) {
          await message.channel.send("<a:Crown:1414236350135144648> **|** Kamu punya role **Boost** dan **Donate**. Ketik `boost` atau `donate` untuk memilih custom role yang ingin dihapus.");
          const collected = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] });
          const typeInput = collected.first().content.toLowerCase();
          if (!['boost', 'donate'].includes(typeInput)) {
            return message.channel.send("<a:Crown:1414236350135144648> **|** Tipe tidak valid. Harus `boost` atau `donate`.");
        ...
        if (deleted) {
          return message.channel.send(`<a:Crown:1414236350135144648> **|** Custom role kamu berhasil dihapus dari server dan database.`);
        } else {
          return message.channel.send(`<a:Crown:1414236350135144648> **|** Data custom role di database dihapus. Role tidak ditemukan di server.`);
        }
      }
      else if (sub === 'edit') {
        const filter = m => m.author.id === message.author.id;
        ...
        if (isBoost && isDonate) {
          await message.channel.send("<a:Crown:1414236350135144648> **|** Kamu punya role **Boost** dan **Donate**. Ketik `boost` atau `donate` untuk memilih custom role yang ingin diedit.");
          const collected = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] });
          const typeInput = collected.first().content.toLowerCase();
          if (!['boost', 'donate'].includes(typeInput)) {
            return message.channel.send("<a:Crown:1414236350135144648> **|** Tipe tidak valid. Harus `boost` atau `donate`.");
        ...
        try {
          await message.channel.send('<a:Crown:1414236350135144648> **|** Apa yang ingin kamu edit? Pilih salah satu: `name`, `color`, atau `icon`.');
          ...
          if (!roleToEdit) {
            return message.channel.send('<a:Crown:1414236350135144648> **|** Role yang ingin kamu edit tidak ditemukan.');
          }

          if (!roleToEdit.editable) {
            return message.channel.send('<a:Crown:1414236350135144648> **|** Saya tidak memiliki izin untuk mengedit role ini.');
          }

          if (editChoice === 'name') {
            await message.channel.send('<a:Crown:1414236350135144648> **|** Ketik nama baru untuk role kamu:');
            ...
            message.channel.send(`<a:Crown:1414236350135144648> **|** Nama role berhasil diubah menjadi **${newRoleName}**.`);

          } else if (editChoice === 'color') {

            await message.channel.send('<a:Crown:1414236350135144648> **|** Ketik warna baru dalam format HEX (contoh: `#FF0000`) atau dua kode HEX untuk gradient (contoh: `#34ebd8 #34eb77`):');
            ...
            if (colorParts.length === 1) {
              if (!/^#([0-9A-F]{6})$/i.test(mainColor)) {
                return message.channel.send('<a:Crown:1414236350135144648> **|** Format warna tidak valid. Gunakan HEX seperti `#RRGGBB`.');
              }
              await roleToEdit.edit({ colors: { primaryColor: mainColor } });
              customRole.roleColor = mainColor;
              await customRole.save();
              message.channel.send(`<a:Crown:1414236350135144648> **|** Warna role berhasil diubah menjadi **${mainColor}**.`);
            } else if (colorParts.length === 2) {
              if (!/^#([0-9A-F]{6})$/i.test(colorParts[0]) || !/^#([0-9A-F]{6})$/i.test(colorParts[1])) {
                return message.channel.send('<a:Crown:1414236350135144648> **|** Kedua warna harus format HEX seperti `#RRGGBB #RRGGBB`.');
              }
              ...
              if (typeof roleToEdit.setColors === 'function') {
                try {
                  ...
                  await customRole.save();
                  message.channel.send(`<a:Crown:1414236350135144648> **|** Gradient role berhasil diubah menjadi **${mainColor} | ${gradientColor}**.`);
                } catch (e) {
                  await roleToEdit.edit({ colors: { primaryColor: mainColor } });
                  customRole.roleColor = mainColor;
                  await customRole.save();
                  message.channel.send('<a:Crown:1414236350135144648> **|** Gagal mengatur gradient, menggunakan warna utama saja.');
                }
              } else {
                await roleToEdit.edit({ colors: { primaryColor: mainColor } });
                customRole.roleColor = mainColor;
                await customRole.save();
                message.channel.send('<a:Crown:1414236350135144648> **|** Gradient tidak didukung, menggunakan warna utama saja.');
              }
            } else {
              return message.channel.send('<a:Crown:1414236350135144648> **|** Masukkan satu atau dua kode warna HEX.');
            }
            ...
            await message.channel.send('<a:Crown:1414236350135144648> **|** Kirim gambar baru untuk icon role kamu (image saja), atau ketik `cancel` untuk batal.');
            ...
            if (iconContent === 'cancel' || !hasAttachment) {
              return message.channel.send('<a:Crown:1414236350135144648> **|** Tidak ada perubahan icon dilakukan.');
            }
            ...
            try {
              ...
              message.channel.send('<a:Crown:1414236350135144648> **|** Icon role berhasil diubah.');
            } catch (err) {
              console.error(err);
              message.channel.send('<a:Crown:1414236350135144648> **|** Gagal mengubah icon. Coba lagi atau pastikan server memiliki boost level yang cukup.');
            }

          } else {
            message.reply("<a:Crown:1414236350135144648> **|** Pilihan tidak valid. Harus `name`, `color`, atau `icon`.");
          }
        } catch (err) {
          console.error(err);
          message.channel.send('<a:Crown:1414236350135144648> **|** Waktu habis atau terjadi kesalahan. Silakan coba lagi.');
          ...
          if (isBoost && isDonate) {
            await message.channel.send("<a:Crown:1414236350135144648> **|** Kamu punya role **Boost** dan **Donate**. Ketik `boost` atau `donate` untuk memilih custom role yang ingin ditambahkan anggota.");
            const collected = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] });
            const typeInput = collected.first().content.toLowerCase();
            if (!['boost', 'donate'].includes(typeInput)) {
              return message.channel.send("<a:Crown:1414236350135144648> **|** Tipe tidak valid. Harus `boost` atau `donate`.");
          ...
          if (customRole.members.length >= maxMembers) {
            return message.reply(`<a:Crown:1414236350135144648> **|** Anda sudah mencapai batas anggota untuk role tipe **${roleType}**.`);
          }

          const target = message.mentions.members.first() || message.guild.members.cache.get(args[1]);

          if (!target) {
            return message.reply('<a:Crown:1414236350135144648> **|** Harap mention user yang ingin Anda tambahkan ke custom role.\nContoh: `seracr adduser @username`');
          }

          if (customRole.members.includes(target.id)) {
            return message.reply('<a:Crown:1414236350135144648> **|** User ini sudah menjadi bagian dari custom role.');
          }

          const role = message.guild.roles.cache.get(customRole.roleId);
          if (!role) {
            return message.reply('<a:Crown:1414236350135144648> **|** Role tidak ditemukan di server.');
          }
          ...
          try {
            ...
            message.channel.send(`<a:Crown:1414236350135144648> **|** <@${target.id}> berhasil ditambahkan ke custom role <@&${role.id}>.`);
          } catch (error) {
            console.error(error);
            message.reply('<a:Crown:1414236350135144648> **|** Gagal menambahkan user ke role. Periksa apakah bot memiliki izin yang cukup.');
          }
        }
            ...
            if (isBoost && isDonate) {
              await message.channel.send("<a:Crown:1414236350135144648> **|** Kamu punya role **Boost** dan **Donate**. Ketik `boost` atau `donate` untuk memilih custom role yang ingin diatur.");
              const collected = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] });
              const typeInput = collected.first().content.toLowerCase();
              if (!['boost', 'donate'].includes(typeInput)) {
                return message.channel.send("<a:Crown:1414236350135144648> **|** Tipe tidak valid. Harus `boost` atau `donate`.");
            ...
            if (!customRole) {
              return message.reply({
                embeds: [
                  new EmbedBuilder()
                    .setColor('Red')
                    .setTitle('<a:Crown:1414236350135144648> **|** Custom Role Tidak Ditemukan')
            ...
            if (customRole.members.length <= 1) {
              return message.reply({
                embeds: [
                  new EmbedBuilder()
                    .setColor('Orange')
                    .setTitle('<a:Crown:1414236350135144648> **|** Tidak Ada Member Lain')
            ...
            await message.channel.send({
              embeds: [
                new EmbedBuilder()
                  .setColor('White')
                  .setTitle('<a:Crown:1414236350135144648> **|** Hapus Member dari Custom Role')
            ...
            try {
              ...
              if (!target) {
                return message.channel.send({
                  embeds: [
                    new EmbedBuilder()
                      .setColor('Red')
                      .setTitle('<a:Crown:1414236350135144648> **|** User Tidak Ditemukan')
              ...
              if (!customRole.members.includes(target.id)) {
                return message.channel.send({
                  embeds: [
                    new EmbedBuilder()
                      .setColor('Red')
                      .setTitle('<a:Crown:1414236350135144648> **|** User Bukan Member Role')
              ...
              if (target.id === message.author.id) {
                return message.channel.send({
                  embeds: [
                    new EmbedBuilder()
                      .setColor('Red')
                      .setTitle('<a:Crown:1414236350135144648> **|** Tidak Bisa Menghapus Diri Sendiri')
              ...
              return message.channel.send({
                embeds: [
                  new EmbedBuilder()
                    .setColor('Green')
                    .setTitle('<a:Crown:1414236350135144648> **|** Member Berhasil Dihapus')
              ...
              return message.channel.send({
                embeds: [
                  new EmbedBuilder()
                    .setColor('Red')
                    .setTitle('<a:Crown:1414236350135144648> **|** Waktu Habis atau Kesalahan')
              ...
              if (isBoost && isDonate) {
                await message.channel.send("<a:Crown:1414236350135144648> **|** Kamu punya role **Boost** dan **Donate**. Ketik `boost` atau `donate` untuk melihat info custom role yang sesuai.");
                const collected = await message.channel.awaitMessages({ filter, max: 1, time: 30000, errors: ['time'] });
                const typeInput = collected.first().content.toLowerCase();
                if (!['boost', 'donate'].includes(typeInput)) {
                  return message.channel.send("<a:Crown:1414236350135144648> **|** Tipe tidak valid. Harus `boost` atau `donate`.");
              ...
              return message.channel.send({ embeds: [embed] });
                ...
                if (!hasPermission) {
                  return message.reply({
                    embeds: [
                      new EmbedBuilder()
                        .setColor('Red')
                        .setTitle('<a:Crown:1414236350135144648> **|** Akses Ditolak')
                ...
                if (!targetUser || !['boost', 'donate'].includes(type)) {
                  return message.reply({
                    embeds: [
                      new EmbedBuilder()
                        .setColor('Red')
                        .setTitle('<a:Crown:1414236350135144648> **|** Format Salah')
                  ...
                  try {
                    ...
                    return message.reply({
                      embeds: [
                        new EmbedBuilder()
                          .setColor('Orange')
                          .setTitle('<a:Crown:1414236350135144648> **|** Role Tidak Bisa Dihapus')
                ...
                return message.reply({
                  embeds: [
                    new EmbedBuilder()
                      .setColor('Green')
                      .setTitle('<a:Crown:1414236350135144648> **|** Custom Role Dihapus')
